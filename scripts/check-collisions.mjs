...
for(;frames<360000;frames++){
 const cart=[...s.cartPos],before=s.workers.map(w=>[...w.pos]);s.advance(1/30);
 ...
 assert.equal(s.cartPos[1],0,'cart may never climb or float');
 assert(segmentClear(cart,s.cartPos,CART_RADIUS),'swept cart footprint clipped a solid');
 for(const w of s.workers){
  const hit=siteSolids.find(b=>intersects(w.pos,.33,1.32,b,b.surface?.3:.18));
  assert(!hit,`worker ${w.id}: ${hit?.id} at ${s.time} ${w.pos}`);
  assert(!built.some(b=>intersects(w.pos,.33,1.32,b)),`worker ${w.id} overlaps a constructed building part`);
 ...
 for(const t of s.tasks.filter(t=>['transfer','handcarry','atLift','atMixer','mixing','ready','landed'].includes(t.supply))){
  ...
  for(const w of s.workers)assert(!intersects(w.pos,.3,1.35,load),`load intersects worker ${w.id} (${t.id}, ${t.supply})`);
 }
